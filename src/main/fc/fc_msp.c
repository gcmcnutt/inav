...
static const char *const flightControllerIdentifier =
    INAV_IDENTIFIER;  // 4 UPPER CASE alpha numeric characters that identify the
                      // flight controller.
...
static serialPort_t *mspFindPassthroughSerialPort(void)
{
    ...
    switch (mspPassthroughMode) {
        ...
        case MSP_PASSTHROUGH_SERIAL_FUNCTION_ID: {
            ...
            if (portConfig) {
                portUsage =
                    findSerialPortUsageByIdentifier(portConfig->identifier);
            }
            break;
        }
    }
...
static void mspFcSetPassthroughCommand(sbuf_t *dst, sbuf_t *src,
                                       mspPostProcessFnPtr *mspPostProcessFn)
{
    const unsigned int dataSize =
        sbufBytesRemaining(src); /* Payload size in Bytes */
...
static void serializeSDCardSummaryReply(sbuf_t *dst)
{
    ...
    sbufWriteU32(dst, sdcard_getMetadata()->numBlocks /
...
static void serializeDataflashSummaryReply(sbuf_t *dst)
{
    ...
    sbufWriteU32(dst, flashfsGetOffset());  // Effectively the current number of
                                            // bytes stored on the volume
#else
    sbufWriteU8(dst, 0);
    sbufWriteU32(dst, 0);
    sbufWriteU32(dst, 0);
...
#ifdef USE_FLASHFS
static void serializeDataflashReadReply(sbuf_t *dst, uint32_t address,
                                        uint16_t size)
{
 ...
 * May set mspPostProcessFunc to a function to be called once the command has
 * been processed
 */
                                   ...
                                   mspPostProcessFnPtr *mspPostProcessFn)
{
    switch (cmdMSP) {
        ...
        case MSP_FC_VARIANT:
            sbufWriteData(dst, flightControllerIdentifier,
        ...
        case MSP_SERVO_CONFIGURATIONS:
            for (int i = 0; i < MAX_SUPPORTED_SERVOS; i++) {
                ...
                sbufWriteU8(dst, 0);
                sbufWriteU8(
                    dst, 255);  // used to be forwardFromChannel, not used
                                // anymore, send 0xff for compatibility reasons
                sbufWriteU32(dst, 0);  // Input reversing is not required since
                                       // it can be done on mixer level
            }
            break;
        ...
        case MSP2_INAV_SERVO_MIXER:
            ...
            for (int i = 0; i < MAX_SERVO_RULES; i++) {
                sbufWriteU8(dst,
                            mixerServoMixersByIndex(nextMixerProfileIndex)[i]
                                .targetChannel);
                sbufWriteU8(dst,
                            mixerServoMixersByIndex(nextMixerProfileIndex)[i]
                                .inputSource);
                sbufWriteU16(
                    dst,
                    mixerServoMixersByIndex(nextMixerProfileIndex)[i].rate);
                sbufWriteU8(
                    dst,
                    mixerServoMixersByIndex(nextMixerProfileIndex)[i].speed);
#ifdef USE_PROGRAMMING_FRAMEWORK
                sbufWriteU8(dst,
                            mixerServoMixersByIndex(nextMixerProfileIndex)[i]
                                .conditionId);
        ...
        case MSP2_COMMON_MOTOR_MIXER:
            for (uint8_t i = 0; i < MAX_SUPPORTED_MOTORS; i++) {
                sbufWriteU16(
                    dst, constrainf(primaryMotorMixer(i)->throttle + 2.0f, 0.0f,
                                    4.0f) *
                             1000);
                sbufWriteU16(dst, constrainf(primaryMotorMixer(i)->roll + 2.0f,
                                             0.0f, 4.0f) *
                                      1000);
                sbufWriteU16(dst, constrainf(primaryMotorMixer(i)->pitch + 2.0f,
                                             0.0f, 4.0f) *
                                      1000);
                sbufWriteU16(dst, constrainf(primaryMotorMixer(i)->yaw + 2.0f,
                                             0.0f, 4.0f) *
                                      1000);
            }
            if (MAX_MIXER_PROFILE_COUNT == 1) break;
            for (uint8_t i = 0; i < MAX_SUPPORTED_MOTORS; i++) {
                sbufWriteU16(
                    dst,
                    constrainf(mixerMotorMixersByIndex(nextMixerProfileIndex)[i]
                                       .throttle +
                                   2.0f,
                               0.0f, 4.0f) *
                        1000);
                sbufWriteU16(
                    dst,
                    constrainf(
                        mixerMotorMixersByIndex(nextMixerProfileIndex)[i].roll +
                            2.0f,
                        0.0f, 4.0f) *
                        1000);
                sbufWriteU16(
                ...
                sbufWriteU16(
                    dst,
                    constrainf(
                        mixerMotorMixersByIndex(nextMixerProfileIndex)[i].yaw +
                            2.0f,
                        0.0f, 4.0f) *
                        1000);
            }
        ...
        case MSP_ANALOG:
            ...
            sbufWriteU16(dst,
                         (uint16_t)constrain(
                             getMAhDrawn(), 0,
                             0xFFFF));  // milliamp hours drawn from battery
            sbufWriteU16(dst, getRSSI());
            sbufWriteU16(dst, (int16_t)constrain(
                                  getAmperage(), -0x8000,
                                  0x7FFF));  // send amperage in 0.01 A steps,
                                             // range is -320A to 320A
            break;

        case MSP2_INAV_ANALOG:
            // Bit 1: battery full, Bit 2: use capacity threshold, Bit 3-4:
            // battery state, Bit 5-8: battery cell count
            sbufWriteU8(dst, batteryWasFullWhenPluggedIn() |
                                 (batteryUsesCapacityThresholds() << 1) |
                                 (getBatteryState() << 2) |
                                 (getBatteryCellCount() << 4));
        ...
        case MSP_RC_TUNING:
            sbufWriteU8(dst, 100);  // rcRate8 kept for compatibity reasons,
                                    // this setting is no longer used
            sbufWriteU8(dst, currentControlRateProfile->stabilized.rcExpo8);
            for (int i = 0; i < 3; i++) {
                sbufWriteU8(
                    dst, currentControlRateProfile->stabilized
                             .rates[i]);  // R,P,Y see flight_dynamics_index_t
            }
        ...
        case MSP2_INAV_RATE_PROFILE:
            ...
            for (uint8_t i = 0; i < 3; ++i) {
                sbufWriteU8(
                    dst, currentControlRateProfile->manual
                             .rates[i]);  // R,P,Y see flight_dynamics_index_t
            }
            break;
        ...
        case MSP_MODE_RANGES:
            for (int i = 0; i < MAX_MODE_ACTIVATION_CONDITION_COUNT; i++) {
                const modeActivationCondition_t *mac =
        ...
        case MSP2_INAV_MISC2:
            ...
            sbufWriteU8(dst, navigationIsControllingThrottle()
                                 ? 1
                                 : 0);  // Auto Throttle Flag (0 or 1)
        ...
        case MSP2_ADSB_VEHICLE_LIST:
            ...
            for (uint8_t i = 0; i < MAX_ADSB_VEHICLES; i++) {
                ...
                sbufWriteU16(dst, (uint16_t)CENTIDEGREES_TO_DEGREES(
                                      adsbVehicle->vehicleValues.heading));
        ...
        case MSP_DEBUG:
            ...
            // debug[x] = ((hse_value / 1000000) * 1000) + (SystemCoreClock /
            // 1000000);         // XX0YY [crystal clock : core clock]

            for (int i = 0; i < 4; i++) {
                sbufWriteU16(dst, debug[i]);  // 4 variables are here for
        ...
        case MSP2_INAV_DEBUG:
            for (int i = 0; i < DEBUG32_VALUE_COUNT; i++) {
                sbufWriteU32(dst, debug[i]);  // 8 variables are here for
                                              // general monitoring purpose
            }
            break;
        ...
        case MSP_CURRENT_METER_CONFIG:
            ...
            sbufWriteU16(dst, constrain(currentBatteryProfile->capacity.value,
                                        0, 0xFFFF));
            break;

        case MSP_MIXER:
            sbufWriteU8(dst, 3);  // mixerMode no longer supported, send 3
                                  // (QuadX) as fallback
            break;
        ...
        case MSP_RX_CONFIG:
            ...
            sbufWriteU8(
                dst, 0);  // for compatibility with betaflight (rcInterpolation)
        ...
        case MSP2_COMMON_SERIAL_CONFIG:
            for (int i = 0; i < SERIAL_PORT_COUNT; i++) {
                ...
                sbufWriteU8(dst,
                            serialConfig()->portConfigs[i].msp_baudrateIndex);
                sbufWriteU8(dst,
                            serialConfig()->portConfigs[i].gps_baudrateIndex);
                sbufWriteU8(
                    dst,
                    serialConfig()->portConfigs[i].telemetry_baudrateIndex);
                sbufWriteU8(
                    dst,
                    serialConfig()->portConfigs[i].peripheral_baudrateIndex);
            }
            break;
        ...
        case MSP_LED_STRIP_CONFIG:
            for (int i = 0; i < LED_MAX_STRIP_LENGTH; i++) {
                ...
                shiftCount += 4;
                legacyLedConfig |= (ledConfig->led_overlay & 0x3F)
                                   << (shiftCount);
                shiftCount += 6;
        ...
        case MSP_BATTERY_STATE:
            ...
            // Battery state
            sbufWriteU8(dst, constrain(getBatteryVoltage() / 10, 0,
                                       255));  // in 0.1V steps
        ...
        case MSP_PID_ADVANCED:
            ...
            /*
             * To keep compatibility on MSP frame length level with Betaflight,
             * axis axisAccelerationLimitYaw limit will be sent and received in
             * [dps / 10]
             */
            sbufWriteU16(
                dst,
                constrain(pidProfile()->axisAccelerationLimitRollPitch / 10, 0,
                          65535));
            sbufWriteU16(
                dst, constrain(pidProfile()->axisAccelerationLimitYaw / 10, 0,
                               65535));
            break;
        ...
        case MSP_NAV_POSHOLD:
            ...
            sbufWriteU16(dst, mixerConfig()->platformType == PLATFORM_AIRPLANE
                                  ? navConfig()->fw.max_auto_climb_rate
                                  : navConfig()->mc.max_auto_climb_rate);
            sbufWriteU16(dst, navConfig()->general.max_manual_speed);
            sbufWriteU16(dst, mixerConfig()->platformType == PLATFORM_AIRPLANE
                                  ? navConfig()->fw.max_manual_climb_rate
                                  : navConfig()->mc.max_manual_climb_rate);
        ...
        case MSP_WP_GETINFO:
            ...
            sbufWriteU8(
                dst,
                NAV_MAX_WAYPOINTS);  // Maximum number of waypoints supported
            sbufWriteU8(dst,
        ...
        case MSP2_INAV_OSD_ALARMS:
            ...
            sbufWriteU16(dst,
                         (int16_t)(osdConfig()->gforce_axis_alarm_min * 1000));
            sbufWriteU16(dst,
                         (int16_t)(osdConfig()->gforce_axis_alarm_max * 1000));
        ...
        case MSP2_INAV_OUTPUT_MAPPING_EXT2: {
            ...
            for (uint8_t i = 0; i < timerHardwareCount; ++i)
                ...
                if (!(timerHardware[i].usageFlags &
                    ...
                    // Eventually, we can try to add more labels for PPM pins,
                    // etc.
                    sbufWriteU8(dst, timerHardware[i].tag == led_tag
                                         ? PIN_LABEL_LED
                                         : PIN_LABEL_NONE);
#endif
                }
        ...
        case MSP2_INAV_ESC_RPM: {
            ...
            for (uint8_t i = 0; i < motorCount; i++) {
                const escSensorData_t *escState =
                    getEscTelemetry(i);  // Get ESC telemetry
        ...
        case MSP2_INAV_RATE_DYNAMICS: {
            sbufWriteU8(
                dst, currentControlRateProfile->rateDynamics.sensitivityCenter);
            sbufWriteU8(dst,
                        currentControlRateProfile->rateDynamics.sensitivityEnd);
            sbufWriteU8(
                dst, currentControlRateProfile->rateDynamics.correctionCenter);
            sbufWriteU8(dst,
                        currentControlRateProfile->rateDynamics.correctionEnd);
            sbufWriteU8(dst,
                        currentControlRateProfile->rateDynamics.weightCenter);
    ...
    if (zoneId < MAX_GEOZONES_IN_CONFIG) {
        ...
        if (vertexIdx >= 0) {
            ...
            if (geoZonesConfig(zoneId)->shape == GEOZONE_SHAPE_CIRCULAR) {
                int8_t vertexRadiusIdx =
                    geozoneGetVertexIdx(zoneId, vertexId + 1);
    ...
    switch (cmdMSP) {
        ...
        case MSP_SET_RAW_RC: {
            ...
            if ((channelCount > MAX_SUPPORTED_RC_CHANNEL_COUNT) ||
                (dataSize > channelCount * sizeof(uint16_t))) {
                return MSP_RESULT_ERROR;
            } else {
        ...
        case MSP_SET_MODE_RANGE:
            ...
            if ((dataSize == 5) &&
                (tmp_u8 < MAX_MODE_ACTIVATION_CONDITION_COUNT)) {
        ...
        case MSP_SET_RC_TUNING:
            if ((dataSize == 10) || (dataSize == 11)) {
                sbufReadU8(src);  // Read rcRate8, kept for protocol
                                  // compatibility reasons
                // need to cast away const to set controlRateProfile
                ((controlRateConfig_t *)currentControlRateProfile)
                    ->stabilized.rcExpo8 = sbufReadU8(src);
                for (int i = 0; i < 3; i++) {
                    tmp_u8 = sbufReadU8(src);
                    if (i == FD_YAW) {
                        ((controlRateConfig_t *)currentControlRateProfile)
                            ->stabilized.rates[i] = constrain(
                            tmp_u8, SETTING_YAW_RATE_MIN, SETTING_YAW_RATE_MAX);
                    } else {
                        ((controlRateConfig_t *)currentControlRateProfile)
                            ->stabilized.rates[i] = constrain(
                            tmp_u8, SETTING_CONSTANT_ROLL_PITCH_RATE_MIN,
                            SETTING_CONSTANT_ROLL_PITCH_RATE_MAX);
                    }
                }
                tmp_u8 = sbufReadU8(src);
                ((controlRateConfig_t *)currentControlRateProfile)
                    ->throttle.dynPID = MIN(tmp_u8, SETTING_TPA_RATE_MAX);
                ((controlRateConfig_t *)currentControlRateProfile)
                    ->throttle.rcMid8 = sbufReadU8(src);
                ((controlRateConfig_t *)currentControlRateProfile)
                    ->throttle.rcExpo8 = sbufReadU8(src);
                ((controlRateConfig_t *)currentControlRateProfile)
                    ->throttle.pa_breakpoint = sbufReadU16(src);
                if (dataSize > 10) {
                    ((controlRateConfig_t *)currentControlRateProfile)
                        ->stabilized.rcYawExpo8 = sbufReadU8(src);
                }
        ...
        case MSP2_INAV_SET_RATE_PROFILE:
            if (dataSize == 15) {
                controlRateConfig_t *currentControlRateProfile_p =
                    (controlRateConfig_t *)
                        currentControlRateProfile;  // need to cast away const
                                                    // to set controlRateProfile
                ...
                currentControlRateProfile_p->throttle.pa_breakpoint =
                    sbufReadU16(src);

                // stabilized
                currentControlRateProfile_p->stabilized.rcExpo8 =
                    sbufReadU8(src);
                currentControlRateProfile_p->stabilized.rcYawExpo8 =
                    sbufReadU8(src);
                for (uint8_t i = 0; i < 3; ++i) {
                    tmp_u8 = sbufReadU8(src);
                    if (i == FD_YAW) {
                        currentControlRateProfile_p->stabilized.rates[i] =
                            constrain(tmp_u8, SETTING_YAW_RATE_MIN,
                ...
                currentControlRateProfile_p->manual.rcYawExpo8 =
                    sbufReadU8(src);
                for (uint8_t i = 0; i < 3; ++i) {
                    tmp_u8 = sbufReadU8(src);
                    if (i == FD_YAW) {
                        currentControlRateProfile_p->manual.rates[i] =
                            constrain(tmp_u8, SETTING_YAW_RATE_MIN,
                                      SETTING_YAW_RATE_MAX);
                    } else {
                        currentControlRateProfile_p->manual.rates[i] =
        ...
        case MSP_SET_MISC:
            if (dataSize == 22) {
                ...
                motorConfigMutable()->mincommand =
                    constrain(sbufReadU16(src), 0, PWM_RANGE_MAX);

                currentBatteryProfileMutable->failsafe_throttle =
                    constrain(sbufReadU16(src), PWM_RANGE_MIN, PWM_RANGE_MAX);
                ...
                if (tmp_u8 <= MAX_SUPPORTED_RC_CHANNEL_COUNT) {
                    ...
                    rxUpdateRSSISource();  // Changing rssi_channel might change
                                           // the RSSI source
                }
                sbufReadU8(src);
...
#ifdef USE_ADC
                batteryMetersConfigMutable()->voltage.scale =
                    sbufReadU8(src) * 10;
                currentBatteryProfileMutable->voltage.cellMin =
                    sbufReadU8(src) * 10;  // vbatlevel_warn1 in MWC2.3 GUI
                currentBatteryProfileMutable->voltage.cellMax =
                    sbufReadU8(src) * 10;  // vbatlevel_warn2 in MWC2.3 GUI
                currentBatteryProfileMutable->voltage.cellWarning =
                    sbufReadU8(src) *
        ...
        case MSP2_INAV_SET_MISC:
            if (dataSize == 41) {
                ...
                currentBatteryProfileMutable->voltage.cellDetect =
                    sbufReadU16(src);
                currentBatteryProfileMutable->voltage.cellMin =
                    sbufReadU16(src);
                currentBatteryProfileMutable->voltage.cellMax =
                ...
                currentBatteryProfileMutable->capacity.warning =
                    sbufReadU32(src);
                currentBatteryProfileMutable->capacity.critical =
                    sbufReadU32(src);
                uint8_t currentCapacityUnit =
                    batteryMetersConfigMutable()->capacity_unit;
                batteryMetersConfigMutable()->capacity_unit = sbufReadU8(src);
                if ((batteryMetersConfig()->voltageSource != BAT_VOLTAGE_RAW) &&
                    (batteryMetersConfig()->voltageSource !=
                     BAT_VOLTAGE_SAG_COMP)) {
                    batteryMetersConfigMutable()->voltageSource =
                        BAT_VOLTAGE_RAW;
                    return MSP_RESULT_ERROR;
                }
                if ((batteryMetersConfig()->capacity_unit !=
                    ...
                    batteryMetersConfigMutable()->capacity_unit =
                        BAT_CAPACITY_UNIT_MAH;
                    return MSP_RESULT_ERROR;
                } else if (currentCapacityUnit !=
                           batteryMetersConfig()->capacity_unit) {
                    if (batteryMetersConfig()->capacity_unit ==
                        BAT_CAPACITY_UNIT_MAH) {
                        osdConfigMutable()->stats_energy_unit =
        ...
        case MSP_SET_SERVO_MIX_RULE:
            ...
            if ((dataSize == 9) && (tmp_u8 < MAX_SERVO_RULES)) {
                customServoMixersMutable(tmp_u8)->targetChannel =
                    sbufReadU8(src);
        ...
        case MSP2_INAV_SET_LOGIC_CONDITIONS:
            ...
            if ((dataSize == 15) && (tmp_u8 < MAX_LOGIC_CONDITIONS)) {
                ...
                logicConditionsMutable(tmp_u8)->operandA.value =
                    sbufReadU32(src);
                logicConditionsMutable(tmp_u8)->operandB.type = sbufReadU8(src);
                logicConditionsMutable(tmp_u8)->operandB.value =
                    sbufReadU32(src);
        ...
        case MSP2_INAV_SET_PROGRAMMING_PID:
            ...
            if ((dataSize == 20) && (tmp_u8 < MAX_PROGRAMMING_PID_COUNT)) {
                ...
                programmingPidsMutable(tmp_u8)->setpoint.value =
                    sbufReadU32(src);
                programmingPidsMutable(tmp_u8)->measurement.type =
                    sbufReadU8(src);
                programmingPidsMutable(tmp_u8)->measurement.value =
                    sbufReadU32(src);
        ...
        case MSP2_COMMON_SET_MOTOR_MIXER:
            ...
            if ((dataSize == 9) && (tmp_u8 < MAX_SUPPORTED_MOTORS)) {
                primaryMotorMixerMutable(tmp_u8)->throttle =
                    constrainf(sbufReadU16(src) / 1000.0f, 0.0f, 4.0f) - 2.0f;
                primaryMotorMixerMutable(tmp_u8)->roll =
                    constrainf(sbufReadU16(src) / 1000.0f, 0.0f, 4.0f) - 2.0f;
                primaryMotorMixerMutable(tmp_u8)->pitch =
                    constrainf(sbufReadU16(src) / 1000.0f, 0.0f, 4.0f) - 2.0f;
                primaryMotorMixerMutable(tmp_u8)->yaw =
                    constrainf(sbufReadU16(src) / 1000.0f, 0.0f, 4.0f) - 2.0f;
            } else
                return MSP_RESULT_ERROR;
            break;
        ...
        case MSP_SET_3D:
            if (dataSize == 6) {
                reversibleMotorsConfigMutable()->deadband_low =
                    sbufReadU16(src);
                reversibleMotorsConfigMutable()->deadband_high =
                    sbufReadU16(src);
        ...
        case MSP_SET_RC_DEADBAND:
            if (dataSize == 5) {
                ...
                rcControlsConfigMutable()->mid_throttle_deadband =
                    sbufReadU16(src);
            } else
                return MSP_RESULT_ERROR;
            break;
        ...
        case MSP_SET_FILTER_CONFIG:
            if (dataSize >= 5) {
                ...
                pidProfileMutable()->dterm_lpf_hz =
                    constrain(sbufReadU16(src), 0, 500);
                pidProfileMutable()->yaw_lpf_hz =
                    constrain(sbufReadU16(src), 0, 255);
                if (dataSize >= 9) {
                    sbufReadU16(src);  // Was gyroConfigMutable()->gyro_notch_hz
                    sbufReadU16(
                        src);  // Was gyroConfigMutable()->gyro_notch_cutoff
                } else {
                    return MSP_RESULT_ERROR;
                }
                if (dataSize >= 13) {
                ...
                if (dataSize >= 17) {
                    sbufReadU16(
                        src);  // Was gyroConfigMutable()->gyro_soft_notch_hz_2
                    sbufReadU16(
                        src);  // Was
                ...
                if (dataSize >= 21) {
                    accelerometerConfigMutable()->acc_notch_hz =
                        constrain(sbufReadU16(src), 0, 255);
                    accelerometerConfigMutable()->acc_notch_cutoff =
                        constrain(sbufReadU16(src), 1, 255);
                } else {
                    return MSP_RESULT_ERROR;
                }
        ...
        case MSP_SET_PID_ADVANCED:
            if (dataSize == 17) {
                sbufReadU16(
                    src);  // pidProfileMutable()->rollPitchItermIgnoreRate
                ...
                sbufReadU8(src);  // BF:
                                  // pidProfileMutable()->vbatPidCompensation
                ...
                /*
                 * To keep compatibility on MSP frame length level with
                 * Betaflight, axis axisAccelerationLimitYaw limit will be sent
                 * and received in [dps / 10]
                 */
                pidProfileMutable()->axisAccelerationLimitRollPitch =
                    sbufReadU16(src) * 10;
                pidProfileMutable()->axisAccelerationLimitYaw =
                    sbufReadU16(src) * 10;
            } else
                return MSP_RESULT_ERROR;
            break;
        ...
        case MSP_SET_INAV_PID:
            if (dataSize == 15) {
                sbufReadU8(
                    src);  // Legacy, no longer in use async processing value
                sbufReadU16(
                    src);  // Legacy, no longer in use async processing value
                sbufReadU16(
                    src);  // Legacy, no longer in use async processing value
...
#endif
#ifdef USE_RANGEFINDER
                rangefinderConfigMutable()->rangefinder_hardware =
                    sbufReadU8(src);
#else
        ...
        case MSP_SET_NAV_POSHOLD:
            if (dataSize == 13) {
                navConfigMutable()->general.flags.user_control_mode =
                    sbufReadU8(src);
                navConfigMutable()->general.max_auto_speed = sbufReadU16(src);
                if (mixerConfig()->platformType == PLATFORM_AIRPLANE) {
                    navConfigMutable()->fw.max_auto_climb_rate =
                        sbufReadU16(src);
                } else {
                    navConfigMutable()->mc.max_auto_climb_rate =
                        sbufReadU16(src);
                }
                navConfigMutable()->general.max_manual_speed = sbufReadU16(src);
                if (mixerConfig()->platformType == PLATFORM_AIRPLANE) {
                    navConfigMutable()->fw.max_manual_climb_rate =
                        sbufReadU16(src);
                } else {
                    navConfigMutable()->mc.max_manual_climb_rate =
                        sbufReadU16(src);
                }
                navConfigMutable()->mc.max_bank_angle = sbufReadU8(src);
                navConfigMutable()->mc.althold_throttle_type = sbufReadU8(src);
                currentBatteryProfileMutable->nav.mc.hover_throttle =
                    sbufReadU16(src);
            } else
                return MSP_RESULT_ERROR;
            break;
        ...
        case MSP_SET_RTH_AND_LAND_CONFIG:
            if (dataSize == 21) {
                ...
                navConfigMutable()->general.flags.rth_climb_first =
                    sbufReadU8(src);
                navConfigMutable()->general.flags.rth_climb_ignore_emerg =
                    sbufReadU8(src);
                navConfigMutable()->general.flags.rth_tail_first =
                    sbufReadU8(src);
                navConfigMutable()->general.flags.rth_allow_landing =
                    sbufReadU8(src);
                navConfigMutable()->general.flags.rth_alt_control_mode =
                    sbufReadU8(src);
                navConfigMutable()->general.rth_abort_threshold =
                    sbufReadU16(src);
                ...
                navConfigMutable()->general.land_slowdown_minalt =
                    sbufReadU16(src);
                navConfigMutable()->general.land_slowdown_maxalt =
                    sbufReadU16(src);
                navConfigMutable()->general.emerg_descent_rate =
                    sbufReadU16(src);
            } else
                return MSP_RESULT_ERROR;
            break;
        ...
        case MSP_SET_FW_CONFIG:
            if (dataSize == 12) {
                currentBatteryProfileMutable->nav.fw.cruise_throttle =
                    sbufReadU16(src);
                currentBatteryProfileMutable->nav.fw.min_throttle =
                    sbufReadU16(src);
                currentBatteryProfileMutable->nav.fw.max_throttle =
                    sbufReadU16(src);
                ...
                currentBatteryProfileMutable->nav.fw.pitch_to_throttle =
                    sbufReadU8(src);
...
#ifdef USE_OPFLOW
                if (dataSize >= 20) {
                    opticalFlowConfigMutable()->opflow_scale =
                        sbufReadU16(src) / 256.0f;
                }
#endif
#ifdef USE_MAG
                if (dataSize >= 22) {
        ...
        case MSP_SET_POSITION_ESTIMATION_CONFIG:
            if (dataSize == 12) {
                positionEstimationConfigMutable()->w_z_baro_p =
                    constrainf(sbufReadU16(src) / 100.0f, 0.0f, 10.0f);
                positionEstimationConfigMutable()->w_z_gps_p =
                    constrainf(sbufReadU16(src) / 100.0f, 0.0f, 10.0f);
                positionEstimationConfigMutable()->w_z_gps_v =
                    constrainf(sbufReadU16(src) / 100.0f, 0.0f, 10.0f);
                positionEstimationConfigMutable()->w_xy_gps_p =
                    constrainf(sbufReadU16(src) / 100.0f, 0.0f, 10.0f);
                positionEstimationConfigMutable()->w_xy_gps_v =
                    constrainf(sbufReadU16(src) / 100.0f, 0.0f, 10.0f);
                gpsConfigMutable()->gpsMinSats =
                    constrain(sbufReadU8(src), 5, 10);
                sbufReadU8(
                    src);  // was
                           // positionEstimationConfigMutable()->use_gps_velned
            } else
                return MSP_RESULT_ERROR;
            break;
        ...
        case MSP_SET_OSD_CONFIG:
            ...
            if ((int8_t)tmp_u8 == -1) {
                ...
                if ((dataSize >= 3) &&
                    (tmp_u8 < OSD_ITEM_COUNT))  // tmp_u8 == addr
                    osdLayoutsConfigMutable()->item_pos[0][tmp_u8] =
                        sbufReadU16(src);
                else
                    return MSP_RESULT_ERROR;
            }
        ...
        case MSP_OSD_CHAR_WRITE:
            if (dataSize >= 55) {
                ...
                for (unsigned ii = 0;
                     ii < MIN(osdCharacterBytes, sizeof(chr.data)); ii++) {
            ...
            if (dataSize >= 2) {
                ...
                if (vtxDevice) {
                    if (vtxCommonGetDeviceType(vtxDevice) != VTXDEV_UNKNOWN) {
                        ...
                        if (newFrequency <=
                            VTXCOMMON_MSP_BANDCHAN_CHKVAL) {  // value is band
                                                              // and channel
                            const uint8_t newBand = (newFrequency / 8) + 1;
                            const uint8_t newChannel = (newFrequency % 8) + 1;
                            if (vtxSettingsConfig()->band != newBand ||
                                vtxSettingsConfig()->channel != newChannel) {
                        ...
                        if (sbufBytesRemaining(src) > 1) {
                            ...
                            if (sbufBytesRemaining(src) > 0) {
                                vtxSettingsConfigMutable()->lowPowerDisarm =
                                    sbufReadU8(src);
                            }

                            // API version 1.42 - extension for pitmode
                            // frequency
                            if (sbufBytesRemaining(src) >= 2) {
                                sbufReadU16(src);  // skip pitModeFreq
                            }

                            // API version 1.42 - extensions for non-encoded
                            // versions of the band, channel or frequency
                            ...
                            if (sbufBytesRemaining(src) >= 3) {
                                ...
                                if (newPowerCount > 0 &&
                                    newPowerCount <
                                        (vtxDevice->capability.powerCount)) {
                                    vtxDevice->capability.powerCount =
                                        newPowerCount;
                                }
                            }
        ...
        case MSP_SET_WP:
            if (dataSize == 21) {
                const uint8_t msp_wp_no =
                    sbufReadU8(src);  // get the waypoint number
                navWaypoint_t msp_wp;
        ...
        case MSP2_COMMON_SET_RADAR_POS:
            if (dataSize == 19) {
                const uint8_t msp_radar_no =
                    MIN(sbufReadU8(src),
                        RADAR_MAX_POIS - 1);  // Radar poi number, 0 to 3
                radar_pois[msp_radar_no].state =
                    sbufReadU8(src);  // 0=undefined, 1=armed, 2=lost
                radar_pois[msp_radar_no].gps.lat =
                    sbufReadU32(src);  // lat 10E7
                radar_pois[msp_radar_no].gps.lon =
                    sbufReadU32(src);  // lon 10E7
                radar_pois[msp_radar_no].gps.alt =
                    sbufReadU32(src);  // altitude (cm)
                radar_pois[msp_radar_no].heading = sbufReadU16(src);  // °
                radar_pois[msp_radar_no].speed = sbufReadU16(src);    // cm/s
                radar_pois[msp_radar_no].lq =
                    sbufReadU8(src);  // Link quality, from 0 to 4
            } else
                return MSP_RESULT_ERROR;
            break;
        ...
        case MSP_SET_MIXER:
            if (dataSize == 1) {
                sbufReadU8(
                    src);  // This is ignored, no longer supporting mixerMode
                mixerUpdateStateFlags();  // Required for correct preset
                                          // functionality
            } else
                return MSP_RESULT_ERROR;
            break;
        ...
        case MSP_SET_RX_CONFIG:
            if (dataSize == 24) {
                ...
                sbufReadU8(src);   // for compatibility with betaflight
                                   // (rcInterpolation)
                sbufReadU8(src);   // for compatibility with betaflight
                                   // (rcInterpolationInterval)
                sbufReadU16(src);  // for compatibility with betaflight
                                   // (airModeActivateThreshold)
                sbufReadU8(src);
                sbufReadU32(src);
                sbufReadU8(src);
                sbufReadU8(src);  // for compatibility with betaflight
                                  // (fpvCamAngleDegrees)
                rxConfigMutable()->receiverType = sbufReadU8(
                    src);  // Won't be modified if buffer is not large enough
            } else
                return MSP_RESULT_ERROR;
            break;
        ...
        case MSP_SET_FAILSAFE_CONFIG:
            if (dataSize == 20) {
                ...
                failsafeConfigMutable()->failsafe_recovery_delay =
                    sbufReadU8(src);
                failsafeConfigMutable()->failsafe_fw_roll_angle =
                    (int16_t)sbufReadU16(src);
                failsafeConfigMutable()->failsafe_fw_pitch_angle =
                    (int16_t)sbufReadU16(src);
                failsafeConfigMutable()->failsafe_fw_yaw_rate =
                    (int16_t)sbufReadU16(src);
                failsafeConfigMutable()->failsafe_stick_motion_threshold =
                    sbufReadU16(src);
                failsafeConfigMutable()->failsafe_min_distance =
                    sbufReadU16(src);
                failsafeConfigMutable()->failsafe_min_distance_procedure =
                    sbufReadU8(src);
            } else
                return MSP_RESULT_ERROR;
            break;
        ...
        case MSP2_COMMON_SET_SERIAL_CONFIG: {
            ...
            while (remainingPortsInPacket--) {
                ...
                portConfig->msp_baudrateIndex =
                    constrain(sbufReadU8(src), BAUD_MIN, BAUD_MAX);
                portConfig->gps_baudrateIndex =
                    constrain(sbufReadU8(src), BAUD_MIN, BAUD_MAX);
                portConfig->telemetry_baudrateIndex =
                    constrain(sbufReadU8(src), BAUD_MIN, BAUD_MAX);
                portConfig->peripheral_baudrateIndex =
                    constrain(sbufReadU8(src), BAUD_MIN, BAUD_MAX);
            }
        } break;
        ...
        case MSP_SET_LED_STRIP_CONFIG:
            if (dataSize == (1 + sizeof(uint32_t))) {
                ...
                ledConfig_t *ledConfig =
                    &ledStripConfigMutable()->ledConfigs[tmp_u8];
        ...
        case MSP2_COMMON_SET_TZ:
            if (dataSize == 2)
                ...
                timeConfigMutable()->tz_offset = (int16_t)sbufReadU16(src);
                timeConfigMutable()->tz_automatic_dst =
        ...
        case MSP2_INAV_OSD_SET_LAYOUT_ITEM: {
            ...
            if (!sbufReadU16Safe(
                    &osdLayoutsConfigMutable()->item_pos[layout][item], src)) {
                return MSP_RESULT_ERROR;
            }
        ...
        case MSP2_INAV_OSD_SET_ALARMS: {
            if (dataSize == 24) {
                ...
                tmp_u16 = sbufReadU16(src);
                osdConfigMutable()->gforce_axis_alarm_min =
                    (int16_t)tmp_u16 / 1000.0f;
                tmp_u16 = sbufReadU16(src);
                osdConfigMutable()->gforce_axis_alarm_max =
                    (int16_t)tmp_u16 / 1000.0f;
...
#ifdef USE_MR_BRAKING_MODE
            if (dataSize == 14) {
                navConfigMutable()->mc.braking_speed_threshold =
                    sbufReadU16(src);
                navConfigMutable()->mc.braking_disengage_speed =
                    sbufReadU16(src);
                ...
                navConfigMutable()->mc.braking_boost_speed_threshold =
                    sbufReadU16(src);
                navConfigMutable()->mc.braking_boost_disengage_speed =
                    sbufReadU16(src);
        ...
        case MSP2_INAV_SET_TEMP_SENSOR_CONFIG:
            if (dataSize == sizeof(tempSensorConfig_t) * MAX_TEMP_SENSORS) {
                for (uint8_t index = 0; index < MAX_TEMP_SENSORS; ++index) {
                    tempSensorConfig_t *sensorConfig =
                    ...
                    tmp_u8 = sbufReadU8(src);
                    sensorConfig->osdSymbol =
                        tmp_u8 > TEMP_SENSOR_SYM_COUNT ? 0 : tmp_u8;
                    for (uint8_t labelIndex = 0;
                         labelIndex < TEMPERATURE_LABEL_LEN; ++labelIndex)
                        sensorConfig->label[labelIndex] =
        ...
        case MSP2_INAV_FWUPDT_EXEC:
            ...
            return MSP_RESULT_ERROR;  // will only be reached if the update is
                                      // not ready
            break;
        ...
        case MSP2_INAV_FWUPDT_ROLLBACK_EXEC:
            ...
            return MSP_RESULT_ERROR;  // will only be reached if the rollback is
                                      // not ready
            break;
#endif
#ifdef USE_SAFE_HOME
        ...
        case MSP2_INAV_SET_FW_APPROACH:
            if (dataSize == 15) {
                uint8_t i;
                if (!sbufReadU8Safe(&i, src) ||
                    i >= MAX_FW_LAND_APPOACH_SETTINGS) {
                    return MSP_RESULT_ERROR;
                }
                fwAutolandApproachConfigMutable(i)->approachAlt =
                    sbufReadU32(src);
                fwAutolandApproachConfigMutable(i)->landAlt = sbufReadU32(src);
                fwAutolandApproachConfigMutable(i)->approachDirection =
                    sbufReadU8(src);

                int16_t head1 = 0, head2 = 0;
                if (sbufReadI16Safe(&head1, src)) {
                    fwAutolandApproachConfigMutable(i)->landApproachHeading1 =
                        head1;
                }
                if (sbufReadI16Safe(&head2, src)) {
                    fwAutolandApproachConfigMutable(i)->landApproachHeading2 =
                        head2;
                }
                fwAutolandApproachConfigMutable(i)->isSeaLevelRef =
                    sbufReadU8(src);
            } else {
                return MSP_RESULT_ERROR;
            }
        ...
        case MSP2_INAV_SET_GEOZONE:
            if (dataSize == 14) {
                ...
                if (!sbufReadU8Safe(&geozoneId, src) ||
                    geozoneId >= MAX_GEOZONES_IN_CONFIG) {
                    return MSP_RESULT_ERROR;
                }
                ...
                geoZonesConfigMutable(geozoneId)->minAltitude =
                    sbufReadU32(src);
                geoZonesConfigMutable(geozoneId)->maxAltitude =
                    sbufReadU32(src);
                geoZonesConfigMutable(geozoneId)->isSealevelRef =
                    sbufReadU8(src);
        ...
        case MSP2_INAV_SET_GEOZONE_VERTEX:
            if (dataSize == 10 || dataSize == 14) {
                ...
                if (geoZonesConfig(geozoneId)->shape ==
                    GEOZONE_SHAPE_CIRCULAR) {
                    if (!geozoneSetVertex(geozoneId, vertexId + 1,
                                          sbufReadU32(src), 0)) {
                        return MSP_RESULT_ERROR;
                    }
                }
        ...
        case MSP2_INAV_SET_RATE_DYNAMICS:
            ...
            if (dataSize == 6) {
                ((controlRateConfig_t *)currentControlRateProfile)
                    ->rateDynamics.sensitivityCenter = sbufReadU8(src);
                ((controlRateConfig_t *)currentControlRateProfile)
                    ->rateDynamics.sensitivityEnd = sbufReadU8(src);
                ((controlRateConfig_t *)currentControlRateProfile)
                    ->rateDynamics.correctionCenter = sbufReadU8(src);
                ((controlRateConfig_t *)currentControlRateProfile)
                    ->rateDynamics.correctionEnd = sbufReadU8(src);
                ((controlRateConfig_t *)currentControlRateProfile)
                    ->rateDynamics.weightCenter = sbufReadU8(src);
                ((controlRateConfig_t *)currentControlRateProfile)
                    ->rateDynamics.weightEnd = sbufReadU8(src);
        ...
        case MSP2_INAV_SET_CUSTOM_OSD_ELEMENTS:
            sbufReadU8Safe(&tmp_u8, src);
            if ((dataSize == (OSD_CUSTOM_ELEMENT_TEXT_SIZE - 1) +
                                 (CUSTOM_ELEMENTS_PARTS * 3) + 4) &&
                (tmp_u8 < MAX_CUSTOM_ELEMENTS)) {
                for (int i = 0; i < CUSTOM_ELEMENTS_PARTS; i++) {
                    ...
                    osdCustomElementsMutable(tmp_u8)->part[i].value =
                        sbufReadU16(src);
                }
                osdCustomElementsMutable(tmp_u8)->visibility.type =
                    sbufReadU8(src);
                osdCustomElementsMutable(tmp_u8)->visibility.value =
                    sbufReadU16(src);
                for (int i = 0; i < OSD_CUSTOM_ELEMENT_TEXT_SIZE - 1; i++) {
                    osdCustomElementsMutable(tmp_u8)->osdCustomElementText[i] =
                        sbufReadU8(src);
                }
                osdCustomElementsMutable(tmp_u8)
                    ->osdCustomElementText[OSD_CUSTOM_ELEMENT_TEXT_SIZE - 1] =
                    '\0';
            } else {
                return MSP_RESULT_ERROR;
            }
...
static bool mspSetSettingCommand(sbuf_t *dst, sbuf_t *src)
{
    ...
    switch (SETTING_TYPE(setting)) {
        ...
        case VAR_STRING: {
            settingSetString(setting, (const char *)sbufPtr(src),
...
static bool mspSettingInfoCommand(sbuf_t *dst, sbuf_t *src)
{
    ...
    // (e.g. a script in the radio) don't need to perform another call to
    // retrieve the value.
    const void *ptr = settingGetValuePointer(setting);
    size_t size = settingGetValueSize(setting);
    sbufWriteDataSafe(dst, ptr, size);
...
void mspWriteSimulatorOSD(sbuf_t *dst)
{
    ...
    // 0 [flags,count] [char] - font bank switch, blink switch and character
    // repeat
    ...
    if (isOSDTypeSupportedBySimulator()) {
               ...
               80)  // whole response should be less 155 bytes at worst.
        {
            ...
            count = 0;
            while (true) {
                displayReadCharWithAttr(osdDisplayPort, osdPos_x, osdPos_y, &c,
                                        &attr);
                if (c == 0) c = 32;

                // REVIEW: displayReadCharWithAttr() should return mode with
                // _TEXT_ATTRIBUTES_BLINK_BIT ! for max7456 it returns mode with
                // MAX7456_MODE_BLINK instead (wrong) because max7456ReadChar()
                // does not decode from MAX7456_MODE_BLINK to
                // _TEXT_ATTRIBUTES_BLINK_BIT it should!
                ...
                if (count == 0) {
                    lastChar = c;
                    blink1 = blink2;
                } else if ((lastChar != c) || (blink2 != blink1) ||
                           (count == 63)) {
                    break;
                }
            ...
            if (count == 1 && cmd == 64) {
                ...
                bytesCount += 2;
            } else if ((count > 2) || (cmd != 0) || (lastChar == 255) ||
                       (lastChar == 0x100) || (lastChar == 0x1ff)) {
                cmd |= count;  // long command for blink/bank switch and symbol
                               // repeat
                sbufWriteU8(dst, 0);
                sbufWriteU8(dst, cmd);
...
bool mspFCProcessInOutCommand(uint16_t cmdMSP, sbuf_t *dst, sbuf_t *src,
                              mspResult_e *ret)
{
    uint8_t tmp_u8;
    ...
    switch (cmdMSP) {
        ...
        case MSP2_COMMON_SETTING:
            *ret =
                mspSettingCommand(dst, src) ? MSP_RESULT_ACK : MSP_RESULT_ERROR;
            break;

        case MSP2_COMMON_SET_SETTING:
            *ret = mspSetSettingCommand(dst, src) ? MSP_RESULT_ACK
                                                  : MSP_RESULT_ERROR;
            break;

        case MSP2_COMMON_SETTING_INFO:
            *ret = mspSettingInfoCommand(dst, src) ? MSP_RESULT_ACK
                                                   : MSP_RESULT_ERROR;
            break;

        case MSP2_COMMON_PG_LIST:
            *ret = mspParameterGroupsCommand(dst, src) ? MSP_RESULT_ACK
                                                       : MSP_RESULT_ERROR;
            break;
        ...
        case MSP2_INAV_OSD_LAYOUTS:
            if (sbufBytesRemaining(src) >= 1) {
                ...
                if (sbufBytesRemaining(src) >= 2) {
                    ...
                    sbufWriteU16(dst,
                                 osdLayoutsConfig()->item_pos[layout][item]);
                } else {
                    // Asking for an specific layout
                    for (unsigned ii = 0; ii < OSD_ITEM_COUNT; ii++) {
                        sbufWriteU16(dst,
                                     osdLayoutsConfig()->item_pos[layout][ii]);
                    }
                }
            } else {
        ...
        case MSP2_INAV_CUSTOM_OSD_ELEMENT: {
            ...
            if (idx < MAX_CUSTOM_ELEMENTS) {
                const osdCustomElement_t *customElement =
                    osdCustomElements(idx);
        ...
        case MSP_SIMULATOR:
            ...
            if (!SIMULATOR_HAS_OPTION(HITL_ENABLE)) {
                ...
                if (dataSize >= 14) {
                    if (feature(FEATURE_GPS) &&
                        ...
                        gpsSolDRV.hdop =
                            gpsSolDRV.fixType == GPS_NO_FIX ? 9999 : 100;
                        ...
                        if (gpsSolDRV.fixType != GPS_NO_FIX) {
                            ...
                            sbufAdvance(
                                src, sizeof(uint32_t) + sizeof(uint32_t) +
                        ...
                        sbufAdvance(
                            src, sizeof(uint8_t) + sizeof(uint8_t) +
                    ...
                    if (sensors(SENSOR_MAG)) {
                        mag.magADC[X] = ((int16_t)sbufReadU16(src)) /
                                        20;  // 16000 / 20 = 800uT
                        mag.magADC[Y] =
                            ((int16_t)sbufReadU16(src)) /
                            20;  // note that mag failure is simulated by
                                 // setting all readings to zero
            ...
            sbufWriteU16(dst,
                         (uint16_t)simulatorData.input[INPUT_STABILIZED_ROLL]);
            sbufWriteU16(dst,
                         (uint16_t)simulatorData.input[INPUT_STABILIZED_PITCH]);
            sbufWriteU16(dst,
                         (uint16_t)simulatorData.input[INPUT_STABILIZED_YAW]);
            sbufWriteU16(
                dst,
                (uint16_t)(ARMING_FLAG(ARMED)
                               ? simulatorData.input[INPUT_STABILIZED_THROTTLE]
                               : -500));
        ...
        case MSP_VTXTABLE_POWERLEVEL: {
            ...
            const uint8_t powerLevel =
                sbufBytesRemaining(src) ? sbufReadU8(src) : 0;
            if (powerLevel == 0 ||
                powerLevel > vtxDevice->capability.powerCount) {
                return MSP_RESULT_ERROR;
            }
            ...
            const uint32_t str_len =
                strnlen(str, 5);  // these _should_ all be null-terminated
            sbufWriteU8(dst, str_len);
            for (uint32_t i = 0; i < str_len; i++) sbufWriteU8(dst, str[i]);
 ...
 */
void mspFcInit(void) { initActiveBoxIds(); }
